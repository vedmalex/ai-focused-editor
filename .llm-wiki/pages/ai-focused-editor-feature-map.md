...
- **Entities** (`entities/<directory>/*.yaml`, one directory per effective type). Shared optional fields: `aliases[]`, `epithets[]`, `speechPatterns[]`, `summary`, `backstory`, `arc`, `notes`. Label field `name` (character/artifact/location) or `term` (author types default to `name`). **Artifacts** add `ownership[]` = `{owner, from?, to?, note?}`. **Discovery** now goes through the `@ai-focused-editor/narrative-knowledge` index (TASK-022) rather than a service-owned scan; `NodeNarrativeEntityService` is a thin RPC adapter over it (see *Services & RPC Map*) and still returns entities alongside `effectiveEntityTypes` + `typeProblems` on the snapshot for its existing frontend consumers. A pre-WP-7 characterization gap is deliberately not reproduced: the old scan emitted three diagnostics (missing entity directory: info; malformed YAML: error; non-object YAML: error) that the thin-adapter path does not surface for this class's eleven still-untouched downstream consumers — recorded as a known gap in a code comment (`node-domain-knowledge-service.ts:234-242`). Created via **New …** commands (`entity-creation.ts`: `createSemanticEntityId` **transliterates Cyrillic→Latin** so a Russian label yields a tag-safe id, `buildEntityYaml`); edited via the **Entity Form Editor**; viewed in **Knowledge Cards**.
...
| `NarrativeEntityBackendService` | `…/narrative-entity` | `NodeNarrativeEntityService` | `BrowserNarrativeEntityService` | Thin adapter (TASK-022 WP-7) over `NarrativeKnowledgeService` in `@ai-focused-editor/narrative-knowledge` — no longer scans `entities/**` itself. Kept only because `relations-map-contribution.ts` and other RPC clients still address this protocol byte-for-byte; `getSnapshot`/`refresh` still return entities + `effectiveEntityTypes` + `typeProblems`, now sourced from the index |
| `NarrativeGraphBackendService` | `…/narrative-graph` | `NodeNarrativeGraphService` | `BrowserNarrativeGraphService` | Thin adapter (TASK-022 WP-7) over `NarrativeKnowledgeService` — timeline / ownership / co-occurrence (cap 20), assembled via the pure `assembleNarrativeGraphSnapshot` from index data, not from its own filesystem scan |
