...
`packages/manuscript-workspace/src/browser/ai-focused-editor-preferences.ts` defines `aiFocusedEditorPreferenceSchema` (title "AI Focused Editor", `scope: PreferenceScope.Folder`) with the AI-connection settings the [[language-models|language model]] needs: `aiFocusedEditor.ai.provider`, `.model`, `.apiKey`, `.endpointUrl`, `.transportKind` (enum `api | proxy | acp | cli | server`), `.transportId`, `.profileId`. It exports `AiFocusedEditorPreferenceContribution`, registered in `manuscript-workspace-frontend-module.ts` via `bind(PreferenceContribution).toConstantValue(AiFocusedEditorPreferenceContribution)`.
