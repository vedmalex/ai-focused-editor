...
## [2026-08-04] maintain | Stale-claim correction after TASK-022 (narrative-knowledge absorption)

Targeted content-accuracy fix on `pages/ai-focused-editor-feature-map.md` — no new source ingested into `raw/`; the correction is driven by this session's own read-only code investigation at repo HEAD (TASK-022 WP-10, IMPLEMENT phase), not a `sources.md`-tracked ingest, so no row was added there. Four spots corrected, all previously describing now-absorbed services as independent scanners: (1) Services & RPC Map row for `NarrativeEntityBackendService` — was "Scan `entities/*` YAML...", now records it as a thin adapter (TASK-022 WP-7) over `NarrativeKnowledgeService` in the new `@ai-focused-editor/narrative-knowledge` package, kept only for RPC-protocol compatibility (`relations-map-contribution.ts` et al.); (2) the same table's `NarrativeGraphBackendService` row, same correction — thin adapter assembling via the pure `assembleNarrativeGraphSnapshot` over index data, not its own scan; (3) the "Project knowledge" → **Entities** bullet's "Discovery is type-aware" sentence, rewritten to match, plus a new honest note that three pre-WP-7 diagnostics (missing entity directory / malformed YAML / non-object YAML) are a deliberately-not-reproduced characterization gap for this class's eleven still-untouched downstream consumers (code comment: `node-domain-knowledge-service.ts:234-242`); (4) the Book Doctor paragraph, appended one sentence naming exactly which four of `gather()`'s eighteen inputs now come from the narrative-knowledge index (via `loadEntityTypes`/`collectExistingEntityCards`/shared parsers) versus the fourteen that still read the filesystem directly. Every claim cross-checked against source in this session (`node-domain-knowledge-service.ts`, `node-narrative-graph-service.ts`, `book-doctor-contribution.ts`); bumped `updated_at` frontmatter. Package-count framing ("three first-party npm packages"), Test Inventory, and all other sections deliberately left untouched — out of scope for this fix.
